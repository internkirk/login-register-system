...
@section('body')
@if (session('link sent successfully'))
<div id="alert-border-3"
    ...
    <div class="ms-3 text-sm font-medium font-vazir">
        @lang('auth.link sent successfully')
    </div>
    <button type="button"
...
<div class="dark:bg-gray-800 mt-8 w-5/6 rounded-2xl mx-auto mb-7 sm:w-4/6 md:w-3/6 lg:w-2/6">
    ...
    <div class="p-4 md:p-5 border border-gray-200 dark:bg-slate-800 dark:border-0 rounded-xl">
        ...
        <div class="text-center text-red-600 font-vazirLight">
            <p>
                @lang('validation.expired_link')
            </p>
        </div>
        @endif
        <form class="space-y-4" action="{{ route('auth.send.magic.request') }}" method="POST">
            @csrf
            <div>
                <p class="text-slate-800 border-b-2 pb-5 dark:text-white border-slate-600  font-vazirLight">
                    @lang('auth.login with magic link')</p>
                <label for="email"
